...
class Logger {
    constructor() {
        ...
        // 环形缓冲区，存储最近的日志条目供 /logs/runtime 查询
        this._ringBuffer = [];
        this._ringBufferMax = 500;
        this._ringSeq = 0;
    }
    ...
    log(level, args, requestId = null) {
        ...
        const entry = {
            ...
            logger: requestId || 'system',
            message: args.map(a => typeof a === 'string' ? a : JSON.stringify(a)).join(' '),
        };
        this._ringBuffer.push(entry);
        if (this._ringBuffer.length > this._ringBufferMax) {
    ...
    /** 查询运行时日志（供 /logs/runtime 接口） */
    getRuntimeLogs({ cursor = 0, limit = 100, level, q } = {}) {
        let entries = this._ringBuffer;
        if (cursor > 0) entries = entries.filter(e => e.seq > cursor);
        if (level) {
        ...
        const nextCursor = sliced.length > 0 ? sliced[sliced.length - 1].seq : cursor;
        return { entries: sliced, nextCursor, bufferSize: this._ringBuffer.length };
    }
}
