...
// 共享 axios 实例缓存，避免每个凭据创建独立的 agent 和 axios
let _sharedAxiosInstance = null;
let _sharedSocialRefreshInstance = null;
...
export class KiroApiService {
    ...
    async initialize() {
        ...
        // 如果凭据已由外部注入（CredentialManager），跳过文件加载
        if (!this._credentialsInjected) {
        ...
        // 注入模式下共享 axios 实例和 HTTP agent
        if (this._credentialsInjected && _sharedAxiosInstance) {
            this.axiosInstance = _sharedAxiosInstance;
            this.axiosSocialRefreshInstance = _sharedSocialRefreshInstance;
        ...
        // 注入模式下缓存为共享实例
        if (this._credentialsInjected) {
            _sharedAxiosInstance = this.axiosInstance;
            _sharedSocialRefreshInstance = this.axiosSocialRefreshInstance;
        }
...
async saveCredentialsToFile(filePath, newData) {
    ...
    async _doTokenRefresh(saveCredentialsToFile, tokenFilePath) {
        try {
            ...
            if (response.data && response.data.accessToken) {
                ...
                // Token 刷新成功（kiro.js 不使用 PoolManager）
            } else {
     ...
     */
    _refreshUuid() {
        // kiro.js 不使用 PoolManager，UUID 刷新由 credential-manager 处理
        logger.warn('[Kiro] UUID refresh not supported in kiro.js standalone mode');
        return null;
    }

    _markCredentialNeedRefresh(reason, error = null) {
        logger.info(`[Kiro] Credential needs refresh. Reason: ${reason}`);
        if (error) error.credentialMarkedUnhealthy = true;
        return true;
    }

    _markCredentialUnhealthy(reason, error = null) {
        logger.info(`[Kiro] Credential marked unhealthy. Reason: ${reason}`);
        if (error) error.credentialMarkedUnhealthy = true;
        return true;
    }
    ...
    _markCredentialUnhealthyWithRecovery(reason, error = null, recoveryTime = null) {
        logger.info(`[Kiro] Credential marked unhealthy with recovery. Reason: ${reason}, Recovery: ${recoveryTime?.toISOString()}`);
        if (error) error.credentialMarkedUnhealthy = true;
        return true;
    }
    ...
    async _handle402Error(error, context = 'unknown') {
        logger.info(`[Kiro] Received 402 (Quota Exceeded) in ${context}. Skipping usage verification.`);
        // 直接标记凭据不健康，不再调用 getUsageLimits 避免卡死
